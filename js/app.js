 ...
 */
    ...
    (() => {
        ...
        function menuInit() {
            ...
            if (menuIcon) document.addEventListener("click", (function(e) {
                const withinBoundaries = e.target.closest(".menu__body") || e.target.closest(".icon-menu");
                if (bodyLockStatus && e.target.closest(".icon-menu")) document.documentElement.classList.toggle("menu-open");
                if (!withinBoundaries && document.documentElement.classList.contains("menu-open")) menuClose();
            }));
        }
        function menuClose() {
        ...
        function formSubmit() {
            ...
            function clearErrorMessages(form) {
                const existingErrorMessages = form.querySelectorAll(".form__error");
                existingErrorMessages.forEach((message => {
                    message.remove();
                }));
            }
            function clearSuccessMessages(form) {
                const existingSuccessMessages = form.querySelectorAll(".form__success");
                existingSuccessMessages.forEach((message => {
                    message.remove();
                }));
            }
            function addErrorMessage(form) {
                clearErrorMessages(form);
                form.querySelectorAll(".line-error").forEach((field => {
                    const errorMessage = document.createElement("p");
                    errorMessage.classList.add("form__error");
                    errorMessage.textContent = "Обов'язково для заповнення";
                    field.appendChild(errorMessage);
                }));
            }
            function addSuccessMessage(form) {
                clearSuccessMessages(form);
                form.querySelectorAll(".line-error").forEach((field => {
                    const successMessage = document.createElement("p");
                    successMessage.classList.add("form__success");
                    successMessage.textContent = "Повідомлення відправленно";
                    field.appendChild(successMessage);
                }));
            }
        }
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            const inputMasksTelegram = document.querySelectorAll(".input-telegram");
            if (inputMasksTelegram.length) inputMasksTelegram.forEach((input => {
                const im = new Inputmask({
                    mask: "@*{1,}",
                    definitions: {
                        "*": {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                new swiper_core_Swiper(".achievements__slider", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    autoHeight: true,
                ...
                new swiper_core_Swiper(".help__slider", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    autoHeight: true,
                ...
                let sliderGallary;
                if (document.querySelector(".popup-gallery__slider")) sliderGallary = new swiper_core_Swiper(".popup-gallery__slider", {
                    ...
                    loop: true,
                    navigation: {
                        nextEl: ".popup__gallery .swiper-button-next",
                        prevEl: ".popup__gallery .swiper-button-prev"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 1,
                ...
                function updateSwiperImages(images) {
                    if (sliderGallary) {
                        while (sliderGallary.slides.length > 0) sliderGallary.removeSlide(0);
                        images.forEach((image => {
                            sliderGallary.addSlide(sliderGallary.slides.length, `<div class="popup-gallery__slide swiper-slide slide"><div class="slide__body"><picture><source srcset="${image}" type="image/webp"><img src="${image}" width="520" alt="05 "></picture></div></div>`);
                            lazyMedia.update();
                        }));
                    }
                }
                document.querySelectorAll(".image-button").forEach((button => {
                    button.addEventListener("click", (function(e) {
                        const imagesString = this.getAttribute("data-photos");
                        if (!imagesString) e.preventDefault();
                        console.log("imagesString:", imagesString);
                        const imagesToLoad = JSON.parse(imagesString);
                        console.log("imagesToLoad:", imagesToLoad);
                        updateSwiperImages(imagesToLoad);
                    }));
                }));
            }
        }
        ...
        const cloneAndInsertMarqueeItem = () => {
            document.addEventListener("DOMContentLoaded", (() => {
                const marquee = document.querySelector(".marquee");
                const marqueeItem = document.querySelector(".marquee__item");
                const clonedItem = marqueeItem.cloneNode(true);
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            function startAnimation() {
                marqueeItem.classList.add("animated");
            }
            window.addEventListener("scroll", (function() {
                if (aboutMeSection && aboutMeSection.getBoundingClientRect().top <= window.innerHeight) {
        ...
        function terxt() {
            ...
            const computedStyles = window.getComputedStyle(helpContainer);
            const margin = parseInt(computedStyles.marginLeft, 10);
            const cards = document.querySelectorAll(".help__item");
            const benefitContent = document.querySelector(".help__content");
            if (!benefitContent) return;
            const gap = parseInt(window.getComputedStyle(benefitContent).marginRight, 10);
            let pixelValues = [];
            cards.forEach(((card, index) => {
                const cardLeft = card.getBoundingClientRect().left;
                const itemDistance = Math.trunc(cardLeft) - margin - index * 10;
                pixelValues[index] = itemDistance;
            }));
            if (!window.matchMedia("(min-width: 960px)").matches) return;
            let tl = gsap.timeline({
                scrollTrigger: {
                    trigger: ".help__container",
                    pin: true,
                    scrub: .1,
                    start: "250px top",
                    end: "+=3000",
                    onUpdate: self => {
                        const currentTransform = Math.abs(gsap.getProperty(helpContainer, "x"));
                        if (currentTransform > gap + 35) benefitContent.classList.add("add-opacity"); else benefitContent.classList.remove("add-opacity");
                    }
                }
            });
            const maxTravelDistance = Math.max(...pixelValues);
            gsap.utils.toArray(".item-help-pc").forEach(((item, i) => {
                const travelDistance = Math.abs(pixelValues[i]);
                const duration = travelDistance / maxTravelDistance * 1;
                tl.to(item, {
                    x: -pixelValues[i],
                    duration,
                    ease: "linear"
        ...
        function ed() {
            ...
            gsap.utils.toArray(".benefit-item").forEach((link => {
                gsap.to(link, {
                    color: "#fff",
                    scrollTrigger: {
                        trigger: link,
                        ...
                        onLeaveBack: () => link.style.color = "rgba(255, 255, 255, 0.10)"
                    }
                });
            }));
        }
        ed();
        cloneAndInsertMarqueeItem();
        window["FLS"] = true;
        isWebp();
        addLoadedClass();
        menuInit();
